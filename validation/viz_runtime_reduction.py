...
parser.add_argument("--fmt", choices=["png", "pdf"], default="pdf")
...
def get_tmins_and_tmaxs_and_yrange(data: DataDType) -> tuple[dict[str, float], dict[str, float], float, float]:
    ...
    for suffix in ["deterministic", "noisy"]:
        with open(f"validation-results/{suffix}.json", mode="r") as f:
            _data = json.load(f)
...
def plot_naive(ax: plt.Axes, deterministic: bool):
    ...
    line, label, t0, t1 = proc_plot_traj(
        ...
        data=data,
        name="naive",
        cumtime_key="actual_cumtime",
        color="black",
        linestyle="dotted",
        label="Actual Runtime (Naïve)",
    )
...
def plot_perf_over_time(
    ...
    for cumtime_key, label, linestyle in zip(
        ...
        [f"Actual Runtime ({ours_label})", f"Simulated Runtime ({ours_label})"],
        [None, "dotted"],
    ):
...
if __name__ == "__main__":
    ...
    if SAVE:
        plt.savefig(f"figs/runtime-reduction.{FMT}", bbox_inches="tight")
    else:
        plt.show()
