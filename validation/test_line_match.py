...
class MyObjectiveFuncSleep:
    ...
    def __call__(self, eval_config: dict[str, float], fidel: int) -> dict[str, float]:
        ...
        print(f"Sleep for {results[RUNTIME_KEY]} seconds")
...
def get_random_opt(wrapper, bench: MFHartmann, seed: int) -> RandomOptimizer:
...
def run_with_wrapper(
    ...
    wrapper = MyObjectiveFuncWrapper(obj_func=bench, ask_and_tell=ask_and_tell, **WRAPPER_KWARGS)
    opt = get_random_opt(wrapper=wrapper, bench=bench, seed=seed)
    start = time.time()
    if ask_and_tell:
...
def run_without_wrapper(bench: MFHartmann, seed: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    start = time.time()
    results = get_random_opt(wrapper=wrapper, bench=bench, seed=seed).optimize()
...
def main(deterministic: bool):
    ...
    for seed in range(SEED):
        ...
        with open(f"validation-results/{suffix}.json", mode="w") as f:
